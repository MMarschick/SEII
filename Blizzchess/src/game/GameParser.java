			...
			if(pieceString!=pieces[0])
			{
				...
				if(attributes.length>6)
				{
					...
					for(int si=0;si<effectList.length;si++)
					{
						...
						{
						case "p": statusEffects.add(new StatusEffect(Ability.POISON, remainingDuration));break;
						case "f": statusEffects.add(new StatusEffect(Ability.FREEZE, remainingDuration));break;
						case "a": statusEffects.add(new StatusEffect(Ability.AVATAR, remainingDuration));break;
